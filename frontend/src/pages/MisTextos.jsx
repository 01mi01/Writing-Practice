...
const MisTextos = ({ onNavigate, onLogout }) => {
    ...
    const fetchPrefsAndTexts = async () => {
      try {
        const [prefsRes, textsRes] = await Promise.all([
          ...
          fetch("http://localhost:3000/api/texts", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  const fmtDate = (val) =>
    new Date(val).toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" });
        ...
        <div className="max-w-7xl mx-auto flex flex-col">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {texts.map((text) => (
                <GlassCard key={text.text_id} className="p-6 shadow-xl flex flex-col gap-4">

                  {/* Header: title + type badge */}
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm font-semibold leading-snug" style={{ color: "var(--text-primary)" }}>
                      <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>Título: </span>
                      {text.title}
                    </p>
                  ...
                  <div className="flex flex-col gap-1">
                    {[
                      ...
                      { label: "Conectores avanzados utilizados", value: text.advanced_connectors_count },
                      { label: "Palabras de la lista de vocabulario", value: text.vocab_words_used },
                      { label: "Errores ortográficos", value: text.spelling_errors },
                    ].map((m) => (
                      <p key={m.label} className="text-sm" style={{ color: "var(--text-secondary)" }}>
                        <span style={{ color: "var(--text-muted)" }}>{m.label}: </span>
                        <span className="font-semibold" style={{ color: "var(--text-primary)" }}>{m.value ?? 0}</span>
                      </p>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                      Fecha de creación: <span style={{ color: "var(--text-secondary)" }}>{fmtDate(text.created_at)}</span>
                    </p>
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                      Última modificación: <span style={{ color: "var(--text-secondary)" }}>{fmtDate(text.updated_at)}</span>
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 mt-auto pt-2">
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: deleting === text.text_id ? "var(--text-muted)" : "#ef4444",
                      }}
